...
int ball::DoBallCollision(ball &b, int l)
{
	if (HasHitBall(b))
	{
		//printf("Reached Hole %i", (l + 1));
		return l++;//HitBall(b);
	}
	else {
		return l;
...
/*
	...
	velocity = parallel + (-perp)*gCoeffRestitution;res
*/
}
...
/*-----------------------------------------------------------
  table class members
  -----------------------------------------------------------*/
void table::SetupCushions()
{
	
	srand(time(NULL));
	tx = TABLE_X * ((rand() % 20)*0.1)+1;
	tz = TABLE_Z * ((rand() % 20)*0.1)+1;

	cushions[0].vertices[0](0) = -tx;
	cushions[0].vertices[0](1) = -tz;
	cushions[0].vertices[1](0) = -tx;
	cushions[0].vertices[1](1) = tz;

	cushions[1].vertices[0](0) = -tx;
	cushions[1].vertices[0](1) = tz;
	cushions[1].vertices[1](0) = tx;
	cushions[1].vertices[1](1) = tz;

	cushions[2].vertices[0](0) = tx;
	cushions[2].vertices[0](1) = tz;
	cushions[2].vertices[1](0) = tx;
	cushions[2].vertices[1](1) = -tz;

	cushions[3].vertices[0](0) = tx;
	cushions[3].vertices[0](1) = -tz;
	cushions[3].vertices[1](0) = -tx;
	cushions[3].vertices[1](1) = -tz;
...
void table::SetupHoles(void)
{
	//generate size
	holes[0].position(0) = (tx * ((rand() % 2)-1)) + HOLE_RADIUS;
	holes[0].position(1) = (tz * ((rand() % 2)-1)) + HOLE_RADIUS;

	if (holes[0].position(0) == holes[0].position(1))
	{

		switch (rand() % 1)
		{
			case 0:
				while (holes[0].position(0) == 0.05f)
				{
					holes[0].position(0) = (tx * ((rand() % 2) - 1)) + HOLE_RADIUS;
				}
				break;
			case 1:
				while (holes[0].position(1) == 0.05f)
				{
					holes[0].position(1) = (tz * ((rand() % 2) - 1)) + HOLE_RADIUS;
				}
				break;
...
void table::Update(int ms)
{
	...
	for(int i=0;i<NUM_BALLS;i++) 
	{
		...
		{
			level = balls[i].DoBallCollision(balls[j], level);
		}
	}
